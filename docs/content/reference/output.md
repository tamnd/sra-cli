...
describes what users get. Pick a format with `-o`, or let sra choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
sra <command> -o table   # aligned columns for reading
sra <command> -o jsonl   # one JSON object per line, for piping
sra <command> -o json    # a single JSON array
sra <command> -o csv     # spreadsheet friendly
sra <command> -o tsv     # tab-separated
sra <command> -o url     # just the URL column
sra <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
sra <command> --fields id,title,url
```
...
```bash
sra <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
sra <command>            # a table, because this is a terminal
sra <command> | wc -l    # JSONL, because this is a pipe
```
